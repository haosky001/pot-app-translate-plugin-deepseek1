...
pub fn translate(
    ...
    let payload = json!({
        ...
        "max_output_tokens": 2048  // DeepSeek 吐字速度在20tps左右，50s内（大约可输出1000Tokens）若无返回则认为超时
    });

    // 发送请求并处理响应; 请求的url == request_url
    let client = reqwest::blocking::ClientBuilder::new()
        .timeout(std::time::Duration::from_secs(50))  // 设置请求超时时间
        .build()?;
    let response = client
        .post(request_url)
    ...
    if response.status().is_success() {
        ...
        // 检查result["choices"][0]["finish_reason"]是否为"content_filter"
        if let Some(finish_reason) = result["choices"][0]["finish_reason"].as_str() {
            if finish_reason == "content_filter" {
                // 如果finish_reason为"content_filter"，返回译文审查错误信息
                return Err("译文存在敏感内容，已经被DeepSeek审查".into());
            }
        }
        ...
        // 检查返回的响应码是否为400且返回的数据为指定的错误信息
        let error_text = response.text()?;
        if error_text == r#"{"detail":"Content Exists Risk"}"# {
            // 如果是原文被审查，返回指定的错误信息
            return Err("输入原文存在敏感内容，已经被DeepSeek审查".into());
        }
        // 如果不满足条件，返回原始的错误信息
        Err(format!("请求失败: {}", error_text).into())
    } else {
...
mod tests {
    ...
    fn try_request() {
        ...
        needs.insert("request_url".to_string(), "https://api.deepseek.com/chat/completions".to_string());
        needs.insert("model_string".to_string(), "deepseek-chat".to_string());
        needs.insert("temperature".to_string(), "0.9".to_string());
